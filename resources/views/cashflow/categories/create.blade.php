    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <h2><i class="fas fa-plus-circle me-2"></i>Nouvelle catégorie</h2>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('cashflow.categories.store') }}">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('categoryType') is-invalid @enderror" 
                                ...
                                <option value="income" {{ old('categoryType') == 'income' ? 'selected' : '' }}>Entrée</option>
                                <option value="expense" {{ old('categoryType') == 'expense' ? 'selected' : '' }}>Sortie</option>
                            </select>
                            @error('categoryType')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('parentCategoryId') is-invalid @enderror" 
                                ...
                                @foreach($parentCategories as $parent)
                                    <option value="{{ $parent->categoryId }}" {{ old('parentCategoryId') == $parent->categoryId ? 'selected' : '' }}>
                                        {{ $parent->categoryName }}
                                    </option>
                                @endforeach
                            </select>
