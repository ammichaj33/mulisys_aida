        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                @can('create-penalties')
                    <form method="POST" action="{{ route('penalties.recalculate-all') }}" class="d-inline"
...
<div class="row mb-4">
    ...
    <div class="col-md-2 mb-3">
        <div class="card stats-card">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0 text-success">{{ $stats['paid'] }}</h3>
    ...
    <div class="col-md-2 mb-3">
        <div class="card stats-card">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0 text-danger">{{ round($stats['total_amount'], 2) }} USD</h3>
    ...
    <div class="col-md-2 mb-3">
        <div class="card stats-card">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0 text-success">{{ round($stats['paid_amount'], 2) }} USD</h3>
    ...
    <div class="col-md-2 mb-3">
        <div class="card stats-card">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="mb-0 text-primary">{{ round($stats['total_amount'] + $stats['paid_amount'], 2) }} USD</h3>
    ...
    <div class="col-md-4 mb-3">
        <div class="card">
            <div class="card-body text-center">
                ...
                <h3 class="text-warning">{{ round($stats['total_amount'], 2) }} USD</h3>
                <small class="text-muted">{{ $stats['pending'] }} pénalité(s)</small>
            </div>
        </div>
    </div>
    <div class="col-md-4 mb-3">
        <div class="card">
            <div class="card-body text-center">
                ...
                <h3 class="text-success">{{ round($stats['paid_amount'], 2) }} USD</h3>
                <small class="text-muted">{{ $stats['paid'] }} pénalité(s)</small>
            </div>
        </div>
    </div>
    <div class="col-md-4 mb-3">
        <div class="card">
            <div class="card-body text-center">
                ...
                <h3 class="text-primary">{{ round($stats['total_amount'] + $stats['paid_amount'], 2) }} USD</h3>
                <small class="text-muted">{{ $stats['total'] }} pénalité(s) au total</small>
            </div>
        </div>
    </div>
</div>
...
<div class="row mb-4">
    <div class="col-12">
        <div class="card" style="background-color: white;">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('penalties.index') }}" class="row g-3">
                    <div class="col-md-3">
                        <label for="status" class="form-label">Statut</label>
                        <select class="form-select" id="status" name="status">
                            <option value="">Tous les statuts</option>
                            <option value="notPaid" {{ (request('status') == 'notPaid') ? 'selected' : '' }}>En attente</option>
                            ...
                            <tbody>
                                @foreach ($penalties as $penalty)
                                    <tr>
                                        ...
                                        <td>{{ $penalty->penaltyMonth }}</td>
                                        <td>
                                            <span class="text-danger fw-bold">{{ round($penalty->amount, 2) }} USD</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('penalties.show', $penalty->penalityId) }}" 
                                               class="btn btn-sm btn-outline-primary" title="Voir détails">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @if($penalty->status === 'notPaid')
                                                <button onclick="payPenalty({{ $penalty->penalityId }}, {{ $penalty->amount }})" 
...
function payPenalty(penaltyId, amount) {
    ...
    document.getElementById('payPenaltyForm').action = @json(url('penalties')) + '/' + penaltyId + '/pay';
