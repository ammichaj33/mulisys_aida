...
class PenaltyCalculationService
{
    ...
    /**
     * Calculate penalty for a specific loan
     */
    public function calculateLoanPenalty(LoanDoc $loan)
    {
        $toleranceDays = config('penalties.tolerance_days', 30);
        $penaltyMonthlyRate = config('penalties.penalty_monthly_rate', 10.0); // 1% par mois (12% annuel ÷ 12)
        $maxPenaltyPercentage = config('penalties.max_penalty_percentage', 10.0);
        ...
        // Date d'octroi du crédit (date de validation)
        ...
        foreach ($interestCalculation['schedule'] as $index => $payment) {
            ...
            if ($actualRepaidBeforeDueDate < $expectedAmountAtDueDate) {
                // Calculer le capital restant dû à cette échéance en tenant compte des paiements partiels
                $remainingCapitalAtDueDate = $this->calculateRemainingCapitalAtDueDate($payment, $repayments, $dueDate, $interestCalculation['schedule']);
                ...
                \Log::info("Échéance " . ($index + 1) . " ({$dueDate->format('d/m/Y')}) - Capital restant: {$remainingCapitalAtDueDate} - Mois de retard: {$monthsOverdue}");

                if ($monthsOverdue > 0 && $remainingCapitalAtDueDate > 0) {
                    // Calculer la pénalité sur le capital restant avec le taux mensuel
                    $monthlyPenalty = $this->calculatePenaltyWithMonthlyRate(
                        $remainingCapitalAtDueDate,
                        $penaltyMonthlyRate,
                        $monthsOverdue
                    );
                    ...
                    if (!$existingPenalty && $monthlyPenalty > 0) {
                        // Créer la pénalité
                        $penalty = Penalty::create([
                            ...
                            'reason' => 'retard',
                            'description' => "Retard échéance " . ($index + 1) . " ({$dueDate->format('d/m/Y')}) - Capital restant: " . round($remainingCapitalAtDueDate, 2) . " USD - " . $monthsOverdue . " mois de retard",
                            'status' => 'notPaid',
                            'created_at' => now(),
                            'createdBy' => Auth::id() ?? 1
                        ]);
                        ...
                        Historique::create([
                            ...
                            'operDescription' => 'Pénalité créée: ' . number_format($monthlyPenalty, 2) . ' USD pour échéance ' . ($index + 1) . ' (' . $monthsOverdue . ' mois de retard)',
                            'userIdFk' => Auth::id() ?? 1
                        ]);
